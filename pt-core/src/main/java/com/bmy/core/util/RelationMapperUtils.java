...
public class RelationMapperUtils {
     ...
     *
     * @param dto ture  : Entity To Dto (defult)
     *            false : Dto To Entry
     *            Rule:
    ...
    public static Object EnAndDtoMapper(Object entry, Object dto, boolean enToDto) throws Exception {
        ...
        //判断类上面是否有自定义注解
        boolean dtoExistAnno = dtoclazz.isAnnotationPresent(RelationMapper.class);
        ...
        for (Method m : (enToDto ? dtos : entrys)) {    //当 enToDto=true 此时是Entity转为Dto，遍历dto的属性
            if ((mName = m.getName()).startsWith("set")) {    //只进set方法
                ...
                for (Field fd : dtofds) {
                    ...
                    if (fd.isAnnotationPresent(RelationMapper.class) || dtoExistAnno) {    //判断field上注解或类上面注解是否存在
                        ...
                        dtoMapName = fd.isAnnotationPresent(RelationMapper.class) ? (fd.getAnnotation(RelationMapper.class).value().toString().equals("") ? fd.getName().toString() : fd.getAnnotation(RelationMapper.class).value().toString()) : fd.getName().toString();
                ...
                if (dtoFieldName != null && !dtoFieldName.equals("null")) {
                    for (Method md : (enToDto ? entrys : dtos)) {
                        if (md.getName().toUpperCase().equals("GET" + dtoFieldName.toUpperCase())) {
                            ...
                            value = (entFieldType.equals(dtoFieldType)) ? md.invoke(enToDto ? entry : dto) : TypeProcessor(entFieldType, dtoFieldType, md.invoke(enToDto ? entry : dto), enToDto);
